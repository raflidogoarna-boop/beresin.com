  ...
  const fetchReviews = async () => {
    try {
      ...
      if (Array.isArray(data)) {
        setReviews(data);
        // Kirim data ke komponen hero/statistik utama secara global
        window.dispatchEvent(new CustomEvent("reviews-updated", { detail: data }));
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(API_URL, {
        ...
        body: JSON.stringify({
          data: [{ nama, rating: rating.toString(), komentar }],
        }),
      });
